...
export default function TenantDashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col md:flex-row relative">
      {/* Mobile Top Navbar */}
      <div className="md:hidden glass-navbar flex items-center justify-between p-4 sticky top-0 z-50">
      ...
      {/* Sidebar */}
      <aside className={`
        ${isMobileMenuOpen ? 'flex absolute top-[65px] left-0 right-0 z-50 glass-heavy pb-4' : 'hidden'} 
        md:flex md:relative md:top-0 md:w-64 glass-heavy border-r border-[var(--separator)] flex-col shrink-0
      `}>
        <div className="hidden md:flex h-16 items-center px-6 border-b border-[var(--separator)]">
          <span className="text-xl font-bold text-gray-900 tracking-tight">Occupyo</span>
        </div>
        <nav className="flex-1 p-4 space-y-1">
          <Link onClick={() => setIsMobileMenuOpen(false)} href="/dashboard/tenant" className="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-xl text-gray-900 bg-black/5 hover:bg-black/10 transition-colors">
        ...
        <div className="hidden md:flex p-4 border-t border-[var(--separator)] items-center gap-3">
          <UserButton />
